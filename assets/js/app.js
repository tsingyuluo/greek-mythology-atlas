...
function openStory(id){const x=s(id);if(!x)return;const guide=STORY_GUIDES[id]||{};const beats=guide.beats||x.chapters.map(c=>'这一章节串联相关人物、场景与原典，可从下方入口继续追踪。');openDrawer(`${img(x.image,'drawer-cover')}<div class="eyebrow">${x.category}</div><h2>${x.title}</h2><div class="alias">${x.subtitle}</div><p>${guide.intro||x.summary}</p><div class="story-reading"><strong>阅读提示</strong><p>${guide.note||'神话通常存在多个来源和版本；下方内容以当前数据的主要叙事线为导读。'}</p></div><div class="drawer-section"><h3>分章叙事</h3><div class="timeline">${x.chapters.map((c,i)=>`<div class="timeline-item"><h4>${String(i+1).padStart(2,'0')} · ${c}</h4><p>${beats[i]||'这一节点连接了人物行动、后果与后续故事。'}</p></div>`).join('')}</div></div><div class="drawer-section"><h3>主要人物</h3><div class="drawer-links">${x.people.map(id=>`<button data-person="${id}">${p(id)?.name||id}</button>`).join('')}</div></div><div class="drawer-section"><h3>古典来源</h3><div class="drawer-links">${x.sources.map(id=>`<button data-source="${id}">${src(id)?.name||id}</button>`).join('')}</div></div>${x.artworks.length?`<div class="drawer-section"><h3>相关名画</h3><div class="drawer-links">${x.artworks.map(id=>`<button data-art="${id}">${a(id)?.title||id}</button>`).join('')}</div></div>`:''}`)}

function openArtwork(id){const x=a(id);if(!x)return;const story=s(x.story);openDrawer(`${img(x.image,'drawer-cover')}<div class="eyebrow">Artwork · ${x.realImage?'公共领域馆藏图':'本地演示图'}</div><h2>${x.title}</h2><div class="alias">${x.en}</div><p>${x.summary}</p><dl class="kv"><dt>艺术家</dt><dd>${x.artist}</dd><dt>年代</dt><dd>${x.date}</dd><dt>馆藏/专题</dt><dd>${x.museum}</dd></dl><div class="drawer-section"><h3>这幅画对应哪个故事？</h3><div class="drawer-links"><button data-story="${x.story}">${story?.title||x.story}</button></div></div><div class="drawer-section"><h3>画面关联人物</h3><div class="drawer-links">${x.people.map(id=>`<button data-person="${id}">${p(id)?.name||id}</button>`).join('')}</div></div><div class="drawer-section"><h3>神话原典</h3><div class="drawer-links">${x.sources.map(id=>`<button data-source="${id}">${src(id)?.name||id}</button>`).join('')}</div></div><div class="notice" style="margin-top:18px">Artwork 只表示后世艺术解释，不会被当作古典 Claim 的事实证据。</div>`)}
