...
const STORY_BEAT_LINKS={
  'trojan-war':[['hera','athena','aphrodite'],['paris'],['paris','aphrodite','hera','athena'],['helen','menelaus','agamemnon'],['achilles','agamemnon','briseis'],['patroclus','hector','andromache'],['priam','hector','achilles'],['odysseus','menelaus','priam','helen']],
  'achilles-wrath':[['agamemnon','briseis','achilles'],['achilles'],['agamemnon','odysseus'],['patroclus','achilles'],['patroclus','hector'],['thetis','achilles'],['achilles','agamemnon'],['achilles','hector','andromache'],['priam','achilles'],['priam','andromache','hector']],
  odyssey:[['odysseus','athena'],['odysseus'],['odysseus','polyphemus','poseidon'],['odysseus'],['circe','hermes','odysseus'],['odysseus'],['odysseus'],['calypso','hermes','zeus'],['telemachus','athena','odysseus'],['penelope','telemachus','odysseus']],
  heracles:[['zeus','hera','heracles'],['hera','heracles'],['heracles','eurystheus'],['heracles'],['heracles'],['heracles','eurystheus'],['deianira','heracles'],['deianira','heracles','hera']],
  perseus:[['danae','perseus'],['danae','zeus','perseus'],['perseus'],['athena','hermes','perseus'],['perseus','medusa'],['perseus','medusa','athena'],['perseus','andromeda'],['danae','perseus']],
  oedipus:[['oedipus','jocasta'],['oedipus'],['oedipus'],['oedipus'],['oedipus','sphinx'],['oedipus','jocasta'],['oedipus','tiresias','jocasta'],['oedipus','tiresias','jocasta']],
  argonauts:[['jason','pelias'],['jason'],['jason','heracles'],['jason','aeetes'],['aeetes','jason'],['medea','jason','aeetes'],['jason','medea','aeetes'],['medea','jason','pelias']],
  'medea-tragedy':[['medea','jason'],['jason','medea','creon'],['creon','medea'],['medea'],['medea','jason'],['medea','creon'],['medea','jason'],['medea'],['medea','aeetes']],
  'persephone-descent':[['persephone','demeter'],['hades','persephone'],['demeter','hecate'],['demeter'],['zeus','hermes','demeter'],['persephone','hades','hermes'],['persephone','demeter','hecate'],['demeter','persephone','hecate']]
};
const ART_HOTSPOTS={
...
function openPerson(id){const x=p(id);if(!x)return;const relations=DATA.people.filter(y=>y.id!==id&&((x.relations||[]).includes(y.id)||(y.relations||[]).includes(id)));openDrawer(`${img(x.image,'drawer-cover')}<div class="eyebrow">${x.type}</div><h2>${x.name}</h2><div class="alias">${x.en} · ${x.grc}</div><p>${x.summary}</p><div class="tags">${pills([x.domain])}</div><div class="drawer-section"><h3>人物故事线</h3><div class="timeline">${x.events.map(e=>`<div class="timeline-item"><h4>${e}</h4><p>关联事件节点 · 可继续追踪来源与参与人物</p></div>`).join('')}</div></div><div class="drawer-section"><h3>参与故事</h3><div class="drawer-links">${x.stories.map(id=>`<button data-story="${id}">${s(id)?.title||id}</button>`).join('')}</div></div><div class="drawer-section"><h3>主要关系 <small>双向识别 ${relations.length}</small></h3><div class="drawer-links">${relations.map(y=>`<button data-person="${y.id}">${y.name}</button>`).join('')||'暂无'}</div></div><div class="drawer-section"><h3>原典来源</h3><div class="drawer-links">${x.sources.map(id=>`<button data-source="${id}">${src(id)?.name||id}</button>`).join('')}</div></div>${x.artworks.length?`<div class="drawer-section"><h3>艺术中的形象</h3><div class="drawer-links">${x.artworks.map(id=>`<button data-art="${id}">${a(id)?.title||id}</button>`).join('')}</div></div>`:''}`)}
...
function openStory(id){
 ...
 const beatLinks=STORY_BEAT_LINKS[id]||[];
